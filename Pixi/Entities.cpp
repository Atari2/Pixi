...
void Sprite::patch(const std::vector<std::string>& extraDefines, Rom& rom, PixiConfig& cfg)
{
	std::string escapedDir = escapeDefines(directory);
	std::string escapedAsmFile = escapeDefines(asm_file);
	std::string escapedAsmDir = escapeDefines(cfg.AsmDir);
	FILE* sprite_patch = fileopen(TEMP_SPR_FILE, "w");
	...
	for (const auto& print : prints) {
		...
		else if (!print.compare(0, 4, "CAPE") && sprite_type == 1)
			ptr_map["cape"] = std::stol(print.substr(4), nullptr, 16);
		else if (!print.compare(0, 9, "CARRIABLE") && sprite_type == 0)
			ptr_map["carriable"] = std::stol(print.substr(9), nullptr, 16);
		else if (!print.compare(0, 7, "CARRIED") && sprite_type == 0)
			ptr_map["carried"] = std::stol(print.substr(7), nullptr, 16);
		else if (!print.compare(0, 6, "KICKED") && sprite_type == 0)
			ptr_map["kicked"] = std::stol(print.substr(6), nullptr, 16);
		else if (!print.compare(0, 5, "MOUTH") && sprite_type == 0)
			ptr_map["mouth"] = std::stol(print.substr(5), nullptr, 16);
		else if (!print.compare(0, 4, "GOAL") && sprite_type == 0)
		...
		table.init = Pointer(ptr_map["init"]);
		table.main = Pointer(ptr_map["main"]);
		if (table.init.is_empty() && table.main.is_empty())
			ErrorState::pixi_error("Sprite {} had neither INIT nor MAIN defined in its file, insertion has been aborted.", asm_file);
		if (sprite_type == 1)
			extended_cape_ptr = Pointer(ptr_map["cape"]);
		else if (sprite_type == 0) {
			ptrs.carried() = Pointer(ptr_map["carried"]);
			ptrs.carriable() = Pointer(ptr_map["carriable"]);
			ptrs.kicked() = Pointer(ptr_map["kicked"]);
			ptrs.mouth() = Pointer(ptr_map["mouth"]);
			ptrs.goal() = Pointer(ptr_map["goal"]);
		}
	}
	if (cfg.Debug) {
		if (sprite_type == 0)
			fmt::print("\tINIT: ${:06X}\n\tMAIN: ${:06X}\n"
				...
				table.init.addr(), table.main.addr(), ptrs.carriable().addr(),
				ptrs.carried().addr(), ptrs.kicked().addr(), ptrs.mouth().addr(), ptrs.goal().addr());
		else if (sprite_type == 1)
			fmt::print("\tINIT: ${:06X}\n\tMAIN: ${:06X}\n\tCAPE: ${:06X}"
				"\n__________________________________\n",
				table.init.addr(), table.main.addr(), extended_cape_ptr.addr());
		else
			fmt::print("\tINIT: ${:06X}\n\tMAIN: ${:06X}\n"
				"\n__________________________________\n",
				table.init.addr(), table.main.addr());
	}
}
